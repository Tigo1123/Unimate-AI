...
import type { StorageService } from '@unimate/storage';
...
export class SourceProcessorService {
  constructor(
    ...
    private readonly storageForProvider: (provider: string) => StorageService,
  ) {}
  ...
  async process(sourceId: string) {
    ...
    const units = await extractDocument(
      await this.storageForProvider(source.storageProvider).get(source.storageKey),
      source.mimeType,
      source.extension,
    );
